              ...
              <form id="formEdit" name="formEdit" class="card-body source-item" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nomor_po" class="form-control" placeholder="#001" value="{{ $purchase->nomor_po }}" />
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group input-group-merge">
                            <input name="tanggal" type="text" class="form-control w-px-150 date-picker" placeholder="YYYY-MM-DD" value="{{ $purchase->tanggal }}" />
                        </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="select form-select" name="status">
                            <option selected disabled value="{{ $purchase->status }}">{{ $purchase->status }}</option>
                    ...
                    <div class="col-12">
                        ...
                        <select class="select2 form-select" id="selectPerusahaan" name="perusahaan_id" required>
                            @foreach ($perusahaan as $ps)
                                <option value="{{ $ps->id }}" @if($purchase->perusahaan_id == $ps->id) selected @endif>{{ $ps->nama_perusahaan }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 col-12">
                        ...
                        <select id="selectKR1" name="pic_1" class="select2 form-select" required>
                            ...
                            @foreach ($karyawan as $kr)
                                <option value="{{ $kr->id }}" @if($purchase->pic_1 == $kr->id) selected @endif>{{ $kr->nama_depan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 col-12">
                        ...
                        <select id="selectKR2" name="pic_2" class="select2 form-select mb-3" required>
                            <option selected disabled>Pilih PIC 1</option>
                            @foreach ($karyawan as $kr)
                                <option value="{{ $kr->id }}" @if($purchase->pic_2 == $kr->id) selected @endif>{{ $kr->nama_depan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                  ...
                  <div class="col-12">
                    ...
                    <select id="selectVendor" name="vendor_id" class="select2 form-select" required>
                        ...
                        @foreach ($vendor as $vd)
                            <option value="{{ $vd->id }}" @if($purchase->vendor_id == $vd->id) selected @endif>{{ $vd->nama_vendor }}</option>
                        @endforeach
                    </select>
                  </div>
                </div>
                                ...
                                <div id="rowAja" class="row w-100 p-3">
                                    <div class="col-md-4">
                                        ...
                                        <select name="item[]" id="getItem1" class="select2 form-select" required>
                                            ...
                                            @foreach ($item as $itm)
                                                <option value="{{ $itm->id }}" @if($it->id == $itm->id) selected @endif>{{ $itm->nama_item }}</option>
                                            @endforeach
                                        </select>
                                    </div>                                
                                    <div class="col-md-3 col-12 mb-md-0 mb-3">
                                        <label class="form-label" for="multicol-phone">Harga</label>
                                        <input type="text" id="getHarga" class="form-control harga" value="{{ $it['harga'] }}" placeholder="Harga item" aria-label="Masukkan quantity" readonly="readonly" />
                                    </div>
                                    <div class="col-md-2 col-12 mb-md-0 mb-3">
                                        <label class="form-label" for="multicol-phone">Quantity</label>
                                        <input type="text" name="quantity[]" class="form-control" placeholder="1" value="{{ $it->pivot->quantity }}" aria-label="Masukkan quantity" />
                                    </div>
                                    <div class="col-md-3 col-12">
                                        <label class="form-label" for="multicol-phone">Total</label>
                                        <input type="text" name="total_harga[]" class="form-control" value="{{ $it->pivot->total_harga }}" placeholder="Total Harga" aria-label="Total Harga" readonly="readonly" />
                                    </div>
                                </div>
                                <div class="d-flex flex-column align-items-center justify-content-between border-start p-2">
                                    <i class="ti ti-x cursor-pointer delete-item" data-item-id="{{ $it->id }}"></i>
                                </div>                                
                            </div>
                        </div>
                    </div>
                @endforeach
                <div id="repeater" class="mb-3" data-repeater-list="group-a"></div>
                <div class="row pb-4">
                    <div class="col-12">
                    ...
                    <div class="col-md-6 d-flex justify-content-end">
                        <div class="invoice-calculations">
                          ...
                          <div class="d-flex justify-content-between mb-2">
                            ...
                            <span class="fw-semibold">{{ $purchase->ppn }}%</span>
                          </div>
                          <hr />
                          <div class="d-flex justify-content-between">
                            <span class="w-px-100">Grand Total:</span>
                            <span class="fw-semibold">{{ $purchase->grand_total }}</span>
                          </div>
                        </div>
                    </div>
                </div>
